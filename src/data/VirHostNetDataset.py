...
class VirHostNetData2(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        ids_A = self.tokenizer(seq_A, max_length=self.max_len, add_special_tokens=True, padding="max_length", truncation=True)
        ids_B = self.tokenizer(seq_B, max_length=self.max_len, add_special_tokens=True, padding="max_length", truncation=True)

        # ids_A = self.tokenizer.batch_encode_plus(seq_A, max_length=self.max_len, add_special_tokens=True, padding="max_length", truncation=True)
        # ids_B = self.tokenizer.batch_encode_plus(seq_A, max_length=self.max_len, add_special_tokens=True, padding="max_length", truncation=True)

        ids_A['input_ids'] = torch.tensor(ids_A['input_ids'])
        ids_A['attention_mask'] = torch.tensor(ids_A['attention_mask'])
        
        ids_B['input_ids'] = torch.tensor(ids_B['input_ids'])
        ids_B['attention_mask'] = torch.tensor(ids_B['attention_mask'])



        if self.labeled:
            # return (ids_A, mask_A, ids_B, mask_B, target)
            return ids_A, ids_B, target
        else:
            # return (ids_A, mask_A, ids_B, mask_B)
            return ids_A, ids_B, {}
